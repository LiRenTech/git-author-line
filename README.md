...
- **Blue (234°)**: Blue is VSCode's symbolic color, representing default commits. It also symbolizes innovation and the future, making it suitable for feature additions.
- **Red (0°)**: Red symbolizes danger, making it appropriate for bug fix commits.
- **Green (100°)**: Green is traditionally used for comments in code and represents safety. Documentation changes are safe and non-intrusive, much like comments.
- **Yellow (60°)**: Yellow is a bright cautionary color that alerts other developers that this code has been refactored, making it suitable for refactor commits.
- **Purple (308°)**: Purple evokes mystery and exploration, reflecting the experimental and investigative nature of testing new functionality, making it appropriate for test commits.
...
- **蓝色 (234°)**: 蓝色是 VSCode 的标志性颜色，代表默认提交，同时蓝色也有创新和未来的含义，所以作为 feat 提交
- **红色 (0°)**: 红色象征危险，所以作为 fix 的 bug 修复提交。
- **绿色 (100°)**: 绿色传统上用于代码中的注释，代表安全。文档更改是安全且非侵入性的，就像注释一样。
- **黄色 (60°)**: 黄色是一种亮眼的警示色，用于提醒其他开发者注意这段代码是被重构了的。所以作为 refactor 提交。
- **紫色 (308°)**: 紫色唤起神秘和探索感，反映了测试新功能的实验性和调查性本质。所以作为 test 提交。
...
- Improved color configurations for commit message matching
- Enhanced status bar icons
...
- Based on regex customizable color configuration
